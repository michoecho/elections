...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (empty($login_err) && empty($password_err) && empty($name_err)) {
		$result =
			...
			array($login, password_hash($password, PASSWORD_DEFAULT), "commission", $name));

		if ($result) {
			$success_info = "Commission '{$name}' registered successfully";
		} else {
			echo "Oops! Something went wrong. Please try again later.";
		}
	}
...
<h1>Add a new commission account</h1>
